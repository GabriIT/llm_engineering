...
stop_remote() {
  case "$MODE" in
    none)
      ;;
    compose)
      "${SSH_CMD[@]}" "cd '$COMPOSE_PROJECT_DIR' && docker compose -f '$COMPOSE_FILE' stop myrag-api"
      ;;
    systemd)
...
start_remote() {
  case "$MODE" in
    none)
      ;;
    compose)
      "${SSH_CMD[@]}" "cd '$COMPOSE_PROJECT_DIR' && docker compose -f '$COMPOSE_FILE' start myrag-api"
      ;;
    systemd)
...
if [[ -n "$REMOTE_REPO_PATH" ]]; then
  ...
  "${SSH_CMD[@]}" "cd '$REMOTE_REPO_PATH' && '$REMOTE_PYTHON_BIN' -m myRAG_app.vector.inspect_cli --db-path '$REMOTE_DB_PATH' --collection '$COLLECTION' --sample 1"
fi
