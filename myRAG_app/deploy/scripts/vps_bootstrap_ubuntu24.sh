...
# Non-destructive Ubuntu 24.04 bootstrap for VPS deployment prerequisites.
# Installs only missing components and avoids replacing globally used runtimes.
...
echo "=== myRAG VPS Bootstrap (Ubuntu 24.04) ==="
echo "User: $(id -un)"
echo "Host: $(hostname)"
...
if [[ "$INSTALL_NGINX" == "no" ]]; then
  ...
  # If another proxy is active, prevent nginx from binding 80/443.
