...
from myRAG_app.parser.parser_config import ParseConfig
from myRAG_app.parser.parsers import parse_knowledge_base
from myRAG_app.parser.reporting import write_parse_report


REPO_ROOT = Path(__file__).resolve().parents[2]
REAL_KNOWLEDGE_ROOT = REPO_ROOT / "myRAG_knowledge"
...
class ParserCliTests(unittest.TestCase):
    def _run_cli(self, args: list[str], cwd: Path) -> subprocess.CompletedProcess:
        return subprocess.run(
            [sys.executable, "-m", "myRAG_app.parser.audit", *args],
            cwd=cwd,
            capture_output=True,
            text=True,
            check=False,
