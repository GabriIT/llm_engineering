...
from myRAG_app.parser.parsers import parse_pptx_file


REPO_ROOT = Path(__file__).resolve().parents[2]
REFERENCE_PPTX = (
    REPO_ROOT / "myRAG_knowledge" / "Metal_Replacement" / "202401 - Metal Replacement.pptx"
)
...
class PptxProbeCliTests(unittest.TestCase):
    def _run_probe(self, args: list[str]) -> subprocess.CompletedProcess:
        return subprocess.run(
            [sys.executable, "-m", "myRAG_app.parser.pptx_probe", *args],
            cwd=REPO_ROOT,
            capture_output=True,
            text=True,
            check=False,
