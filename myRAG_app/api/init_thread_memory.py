...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Initialize/check PostgreSQL + pgvector schema for myRAG thread memory "
            "using MYRAG_THREADS_* environment variables."
        )
    )
    parser.add_argument(
        "--json",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not health.get("enabled", False):
        print("Thread memory is disabled. Set MYRAG_THREADS_ENABLED=1 to enable.")
        return 2
