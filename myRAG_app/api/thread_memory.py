...
from myRAG_app.vector.config import DEFAULT_EMBEDDING_MODEL
...
class ThreadMemoryConfig:
    ...
    def from_env(cls) -> "ThreadMemoryConfig":
        ...
        dsn_raw = _env_first(("MYRAG_THREADS_DB_DSN", "MYRAG_THREADS_DSN"), "")
        return cls(
            enabled=_bool_env("MYRAG_THREADS_ENABLED", False),
            dsn=dsn_raw.strip() or None,
            host=_env_first(("MYRAG_THREADS_DB_HOST", "MYRAG_THREADS_HOST"), "127.0.0.1"),
            port=_int_env(
                "MYRAG_THREADS_DB_PORT", _int_env("MYRAG_THREADS_PORT", 5432, 1, 65535), 1, 65535
            ),
            dbname=_env_first(("MYRAG_THREADS_DB_NAME", "MYRAG_THREADS_DB"), "myRAG_threads"),
            user=_env_first(("MYRAG_THREADS_DB_USER", "MYRAG_THREADS_USER"), "postgresql"),
            password=_env_first(
                ("MYRAG_THREADS_DB_PASSWORD", "MYRAG_THREADS_PASSWORD"), "postgresql"
            ),
            sslmode=os.getenv("MYRAG_THREADS_DB_SSLMODE", "disable"),
            connect_timeout=_int_env("MYRAG_THREADS_DB_CONNECT_TIMEOUT", 5, 1, 60),
            embedding_model=_env_first(
                ("MYRAG_THREADS_EMBEDDING_MODEL",), DEFAULT_EMBEDDING_MODEL
            ),
            vector_dims=_coerce_vector_dims(
                _int_env(
                    "MYRAG_THREADS_VECTOR_DIMS",
                    _int_env("MYRAG_THREADS_EMBEDDING_DIMS", 3072, 128, 8192),
                    128,
                    8192,
                )
            ),
            recent_messages=_int_env("MYRAG_THREADS_RECENT_MESSAGES", 10, 1, 100),
            semantic_top_k=_int_env("MYRAG_THREADS_SEMANTIC_TOP_K", 6, 0, 50),
            max_history_messages=_int_env(
                "MYRAG_THREADS_MAX_HISTORY_MESSAGES",
                _int_env("MYRAG_THREADS_MAX_HISTORY", 24, 1, 120),
                1,
                120,
            ),
        )
...
class ThreadMemoryStore:
    ...
    def ensure_schema(self) -> bool:
        ...
        CREATE TABLE IF NOT EXISTS myrag_thread_sessions (
        ...
        CREATE TABLE IF NOT EXISTS myrag_thread_messages (
          ...
          CONSTRAINT fk_thread_session
            ...
            REFERENCES myrag_thread_sessions (username, thread_id)
            ON DELETE CASCADE
        );

        CREATE INDEX IF NOT EXISTS idx_myrag_thread_messages_lookup
          ON myrag_thread_messages (username, thread_id, created_at, id);
        """
        try:
            with self._connect() as conn:
                with conn.cursor() as cur:
                    ...
                    if dims <= 2000:
                        cur.execute(
                            """
                            CREATE INDEX IF NOT EXISTS idx_myrag_thread_messages_embedding
                              ON myrag_thread_messages USING ivfflat (embedding vector_cosine_ops)
                              WITH (lists = 100);
                            """
                        )
    ...
    def _upsert_thread(
        ...
        *,
        ...
        with conn.cursor() as cur:
            cur.execute(
                """
                INSERT INTO myrag_thread_sessions (username, thread_id, title)
                VALUES (%s, %s, %s)
        ...
        *,
        ...
        with conn.cursor() as cur:
            if embedding_literal:
                cur.execute(
                    """
                    INSERT INTO myrag_thread_messages
